...
import { z } from "zod";
import { Shield, Loader2, CheckCircle2, AlertCircle, AlertTriangle, Trash2 } from "lucide-react";

type FirewallDeleteProps = {
  firewallId?: string;
  firewallName?: string;
};

const FirewallDelete: React.FC<FirewallDeleteProps> = (props) => {
  const { firewallId, firewallName } = props || {};
  ...
  const displayName = firewallName || firewallId || "unknown";
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Error deleting firewall");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="w-full max-w-md bg-[#0d1117] border border-gray-700 rounded-lg overflow-hidden text-gray-100">
        <div className="p-6 text-center">
          ...
          <h3 className="text-xl font-semibold mb-2">Firewall Deleted</h3>
          <p className="text-gray-400">The firewall has been removed.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-full max-w-md bg-[#0d1117] border border-gray-700 rounded-lg overflow-hidden text-gray-100">
        <div className="p-6 text-center">
          ...
          <h3 className="text-xl font-semibold mb-2">Firewall Not Specified</h3>
          <p className="text-gray-400">Please specify a firewall ID or name to delete.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full max-w-md bg-[#0d1117] border border-red-900/50 rounded-lg overflow-hidden text-gray-100">
      <div className="px-6 py-4 border-b border-gray-700 bg-red-950/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-red-400">Delete Firewall</h2>
            <p className="text-sm text-gray-400">Resources will be unprotected</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="text-sm text-gray-300 space-y-2">
          <p>Deleting this firewall will:</p>
          <ul className="list-disc list-inside text-gray-400 space-y-1">
            <li>Remove all firewall rules</li>
            <li>Leave attached resources unprotected</li>
          </ul>
        </div>
...
export const firewallDeleteComponent: TamboComponent = {
  name: "firewallDelete",
  description: "Render a confirmation dialog to delete a firewall. ALWAYS render this when user wants to delete a firewall.",
  component: FirewallDelete,
