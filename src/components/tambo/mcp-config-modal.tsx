...
export const McpConfigModal = ({
  ...
  const addServer = (e: React.FormEvent) => {
    ...
    if (serverUrl.trim()) {
      const serverConfig = {
        ...
        ...(authHeader.trim() ? { headers: { Authorization: authHeader.trim() } } : {}),
      };
  ...
  const getServerInfo = (server: McpServerInfo) => {
    if (typeof server === "string") {
      return { url: server, transport: "HTTP (default)", name: null, hasAuth: false };
    } else {
      const headers = (server as McpServerInfo & { headers?: Record<string, string> }).headers;
      return {
        url: server.url,
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 border border-muted rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-150 text-sm font-mono"
                />
                <p className="mt-1 text-xs text-muted-foreground">
                  Will be sent as the Authorization header with each request
                </p>
              </div>
